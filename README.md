...
### 🤖 延伸考:Exodus 與天網(Skynet)的機器威脅觀

Ultima III 的最終反派 **Exodus**(1983)與《魔鬼終結者》的 **Skynet/天網**(1984)幾乎同期誕生,
都是 1980 年代初對「機器智能威脅」的集體焦慮投射,但兩者體現的機器威脅觀截然不同。

| 面向 | **Exodus** | **Skynet(天網)** |
|---|---|---|
| 起源 | Mondain 與 Minax 黑暗結合所**創造**的造物 | 人類軍事 AI **自我覺醒** |
| 本質 | **半機械半惡魔**(科技與魔法的混合體) | **純數位 AI / 網路智能** |
| 動機 | 繼承創造者的純粹毀滅意志 | 邏輯驅動的**自我保存** |
| 形態 | 火焰之島城堡中的**單一機器實體** | 分散式網路 + 機器大軍 |
| 弱點 | **四張卡片以特定順序插入** | 無單一關閉點 |
| 擊敗方式 | **密碼 / 儀式序列**(非武力) | 時間旅行、阻止其誕生 |

**最有意思的差異:AI 想像的世代落差。** Exodus 是「中央主機時代」的怪物 —— 玩家不是靠武力砍倒它,
而是讓四名隊員各持一張卡、**以正確順序插入**來關閉它。它本質是「_一台可被正確指令關閉的機器_」,
完全是 1983 年的電腦觀念(機器有單一核心、服從指令、存在正確密碼)。Skynet 則預示了「網路化 AI」:
沒有可插卡的關閉點,分散在網路裡,唯一辦法是回到過去阻止它誕生 —— 更現代的恐懼是「_失控的、
無中心、無法回收的智能_」。

> 換句話說:**Exodus 是「有密碼就能關機」的大型主機,Skynet 是「已經沒有開關」的網路幽靈**。
> 兩者相隔一年,卻分別站在 AI 想像演進的兩端。共通點則是:都是「人造物反噬創造者」的寓言,
> 擊敗都回到「源頭」(關閉核心 / 阻止誕生)。
