...
- **底部文字滾動區中文亂碼根因已修正**（見下節「底部文字協定說明」）：戰鬥/方向/狀態訊息現正確顯示中文（「玩家」「命中!」「匕首 攻擊」「未命中!」「北/方向」「等待」等）。
- 無法力(non-mage)職業在隊伍面板法力欄的標籤,**現顯示「不可施法」**(P15;早期曾為「非法師」,
  生硬,已淘汰)。AppImage 實機截圖確認為「不可施法」。
...
**驗證**：`U3_DBG_TEXT=1` 跑戰鬥，含控制碼字串數 `392 → 0`；截圖底部正確顯示「玩家」「命中!」「匕首 攻擊」「未命中!」「北/方向」「等待」(P15 後 Pass 由「通過」改「等待」)。
